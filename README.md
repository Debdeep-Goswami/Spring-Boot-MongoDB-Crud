...
||
|  ---------    |
| **_Note_** : In `SpringBootMongoDBApplication.java` class we have autowired both User and Employee repositories. <br/>If there is no record present in DB for any one of that module class (Employee or User), static data is getting inserted in DB from `HelperUtil.java` class when we are starting the app for the first time.| 
...
4. #### CRUD operation for Super Heroes
    ...
    In **com.spring.mongo.demo.controller.UserController.java** class, 
    ...
    In **com.spring.mongo.demo.repository.UserRepository.java**, we are extending `MongoRepository<Class, ID>` interface which enables CRUD related methods.
    ```
    public interface UserRepository extends MongoRepository<User, String> {
    }
    ```
   
   In **com.spring.mongo.demo.service.impl.UserServiceImpl.java**, we are autowiring above interface using `@Autowired` annotation and doing CRUD operation.
