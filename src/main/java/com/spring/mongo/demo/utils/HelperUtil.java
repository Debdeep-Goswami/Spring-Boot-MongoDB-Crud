...
import com.spring.mongo.demo.model.User.role;
...
public class HelperUtil {
    ...
    public static Supplier<List<User>> UsersSupplier = () ->
            Arrays.asList(
                    User.builder().first_name("Wade").last_Name("Deadpool").Email("shashu@gmail.com").Phone("04587569864").user_name("user123").password_encrypted(new char[]{'a', 'b'}).state("Telangana").country("India").role(role.CONSUMER).gender(User.gender.M).status(User.StatusEnum.ACTIVE).build()
            );
